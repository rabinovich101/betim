...
export default function ExpandableBettingMarkets({ event, matchId }: ExpandableBettingMarketsProps) {
  ...
  const renderSpecialMarkets = () => {
    ...
    // Asian Handicap
    if (event.markets.asianHandicap) {
      specialMarkets.push(
        <div key="asianHandicap" className="mb-6">
          <h4 className="text-sm font-bold text-[#00ff87] mb-3">Asian Handicap</h4>
          <div className="grid grid-cols-2 gap-2">
            {Object.entries(event.markets.asianHandicap).map(([line, odds]: [string, any]) => (
              <div key={line} className="contents">
                <button
                  onClick={() => handleBetClick('asianHandicap', `${line}-home`, odds.home, 
                    `${event.homeTeam} ${line}`)}
                  className={`p-3 rounded-lg transition-all text-sm ${
                    isBetSelected(matchId, `asianHandicap-${line}-home`)
                      ? 'bg-[#00ff87] text-[#0a1a1f] font-bold'
                      : 'bg-[#1a1a2e] text-white hover:bg-[#00ff87]/20 hover:text-[#00ff87] border border-white/10'
                  }`}
                >
                  <div className="text-xs opacity-70">{event.homeTeam} {line}</div>
                  <div className="font-bold">{odds.home}</div>
                </button>
                <button
                  onClick={() => handleBetClick('asianHandicap', `${line}-away`, odds.away, 
                    `${event.awayTeam} ${parseFloat(line) > 0 ? '-' : '+'}${Math.abs(parseFloat(line))}`)}
                  className={`p-3 rounded-lg transition-all text-sm ${
                    isBetSelected(matchId, `asianHandicap-${line}-away`)
                      ? 'bg-[#00ff87] text-[#0a1a1f] font-bold'
                      : 'bg-[#1a1a2e] text-white hover:bg-[#00ff87]/20 hover:text-[#00ff87] border border-white/10'
                  }`}
                >
                  <div className="text-xs opacity-70">{event.awayTeam} {parseFloat(line) > 0 ? '-' : '+'}{Math.abs(parseFloat(line))}</div>
    ...
    if (event.sport === 'tennis') {
      if (event.markets.totalGames) {
        specialMarkets.push(
          renderMarketSection('Total Games', event.markets.totalGames, 'totalGames')
        );
      }
      if (event.markets.setWinner) {
        specialMarkets.push(
          renderMarketSection('Set Winner', event.markets.setWinner, 'setWinner')
        );
      }
      if (event.markets.setBetting) {
        specialMarkets.push(
          renderMarketSection('Set Betting', event.markets.setBetting, 'setBetting')
        );
      }
    }
  ...
  return (
    <>
      ...
      {isExpanded && (
        <div className="absolute left-0 right-0 top-full mt-2 p-6 bg-gradient-to-br from-[#1f2937] to-[#111827] rounded-xl border border-white/10 shadow-2xl z-10">
          {/* Main markets */}
