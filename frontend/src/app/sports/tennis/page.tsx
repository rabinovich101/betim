...
export default function TennisPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        // Fetch only tennis events
        const tennisEvents = await oddsApi.getEventsBySport('tennis');
        
        // Sort by start time (live first, then upcoming)
        const sortedEvents = tennisEvents.sort((a, b) => {
          // Live games first
          if (a.isLive && !b.isLive) return -1;
          if (!a.isLive && b.isLive) return 1;
          
          // If both are live, sort by current set
          if (a.isLive && b.isLive) {
            return (b.currentSet || 0) - (a.currentSet || 0);
          }
  ...
  const handleBetClick = (event: Event, betType: '1' | '2') => {
    if (!event.markets.matchWinner) return;

    const odds = betType === '1' 
      ? event.markets.matchWinner.player1 
      : event.markets.matchWinner.player2;
    
    const selection = betType === '1' 
      ? event.player1 
      : event.player2;

    addBet({
      id: `tennis-${event.id}-${betType}`,
      matchId: `tennis-${event.id}`,
      selection: selection || '',
      type: betType,
      odds,
      teams: `${event.player1} vs ${event.player2}`,
      sport: 'tennis'
    });
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-[#0f0f23] to-[#1a1a2e] p-6">
      ...
      <div className="mb-8">
        <div className="flex items-center gap-3 mb-2">
          ...
          <h1 className="text-4xl font-bold text-white">Tennis</h1>
        </div>
        <p className="text-[#a0a0b8]">All tennis matches • {allMatches.length} events available</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
        ...
        <div className="bg-gradient-to-br from-[#1f2937]/50 to-[#111827]/50 rounded-lg p-4 border border-white/5">
          <div className="flex items-center gap-3">
            <span className="text-2xl">🏆</span>
            <div>
              <p className="text-[#a0a0b8] text-xs">Tournaments</p>
              <p className="text-white font-bold text-lg">
                {new Set(allMatches.map(m => m.tournament)).size}
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="bg-gradient-to-br from-[#1f2937] to-[#111827] rounded-xl p-6 border border-white/5">
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-2xl font-bold text-white">All Tennis Matches</h2>
        ...
        {displayedMatches.length === 0 ? (
          <p className="text-center text-[#a0a0b8] py-8">No tennis matches available at the moment</p>
        ) : (
                    ...
                    <div className="flex items-center gap-4">
                      <div className="flex-1">
                        <div className="flex items-center gap-3">
                          ...
                          {match.player1Country && (
                            <span className="text-xs text-[#a0a0b8]">{match.player1Country}</span>
                          )}
                        </div>
                        <div className="flex items-center gap-3 mt-1">
                          <span className="text-white font-medium">{match.player2}</span>
                          {match.player2Country && (
                            <span className="text-xs text-[#a0a0b8]">{match.player2Country}</span>
                          )}
                        </div>
                      </div>
                      ...
                      {match.isLive && (
                        <div className="text-center px-4">
                          {match.player1Sets !== undefined && match.player2Sets !== undefined && (
                            <div>
                              <div className="text-xs text-[#a0a0b8] mb-1">Sets</div>
                              <div className="text-xl font-bold text-[#00ff87]">
                                {match.player1Sets} - {match.player2Sets}
                              </div>
                            </div>
                          )}
                          {match.player1Games !== undefined && match.player2Games !== undefined && (
                            <div className="mt-2">
                              <div className="text-xs text-[#a0a0b8]">Games</div>
                    ...
                    <div className="flex gap-2 ml-4">
                      <button 
                        ...
                        className={`px-4 py-3 min-w-[80px] rounded-lg transition-all text-sm font-bold ${
                          isBetSelected(`tennis-${match.id}`, '1')
                            ? 'bg-[#00ff87] text-[#0a1a1f]'
                            : 'bg-[#1a1a2e] text-white hover:bg-[#00ff87]/20 hover:text-[#00ff87] border border-white/10'
                        }`}
                      >
                        <div className="text-xs opacity-70">P1</div>
                        <div>{match.markets.matchWinner.player1}</div>
                      </button>
                      <button 
                        onClick={() => handleBetClick(match, '2')}
                        className={`px-4 py-3 min-w-[80px] rounded-lg transition-all text-sm font-bold ${
                          isBetSelected(`tennis-${match.id}`, '2')
                            ? 'bg-[#00ff87] text-[#0a1a1f]'
                            : 'bg-[#1a1a2e] text-white hover:bg-[#00ff87]/20 hover:text-[#00ff87] border border-white/10'
                        }`}
                      >
                        <div className="text-xs opacity-70">P2</div>
                        <div>{match.markets.matchWinner.player2}</div>
                      </button>
